        ...
        try:
            ...
            async for chunk in llm.astream(messages):
                ...
                if not text:
                    continue
                buffer += text
                # Filter out <think>...</think> blocks token by token
                while True:
                    if in_think:
                        ...
                        if end != -1:
                            ...
                            buffer = ""  # discard buffered think content
                            break
                    else:
                        ...
                        if start != -1:
                            ...
                            # No think tag — safe to emit, but hold last 8 chars
                            # in case a tag starts at the boundary
                            if len(buffer) > 8:
                                safe = buffer[:-8]
                                buffer = buffer[-8:]
